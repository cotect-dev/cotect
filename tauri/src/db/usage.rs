...
pub fn query(c: &Conn, f: &UsageFilter) -> Result<Vec<UsageRecord>> {
    ...
    if let Some(v) = f.from_ts {
        sql.push_str(" AND ts >= ?");
        params.push(Box::new(v));
    }
    if let Some(v) = f.to_ts {
        sql.push_str(" AND ts < ?");
        params.push(Box::new(v));
    }
    if let Some(v) = &f.provider_id {
...
fn p50_for_bucket(
    ...
    let (col_filter, value) = match group_by {
        GroupBy::Provider => (
            "provider_id =",
            bucket.trim_start_matches("provider:").to_string(),
        ),
    ...
    let times: Vec<i64> = stmt
        .query_map(rusqlite::params_from_iter(p_refs.iter().copied()), |r| {
            r.get::<_, i64>(0)
        })?
