...
pub fn diagnostic(err: &ProbeError, endpoint: &str) -> String {
    match err {
        ...
        ProbeError::Auth(msg) if endpoint.contains("api.openai.com") => {
            format!(
                "API key missing or invalid. OpenAI keys start with `sk-`. ({})",
                msg
            )
        }
        ProbeError::Auth(msg) => format!("Authentication failed. Check the API key. ({})", msg),
        ProbeError::NotFound(_) => {
            "Server doesn't expose `/v1/models`. Try a different endpoint or enable OpenAI-compat."
                .into()
        }
        ProbeError::Tls(msg) => format!(
            "TLS handshake failed. If self-signed, configure trust at the OS level. ({})",
            msg
        ),
...
fn detect_server_type(headers: &reqwest::header::HeaderMap, body_sample: &str) -> String {
    if let Some(server) = headers.get("server").and_then(|v| v.to_str().ok()) {
        if server.to_lowercase().contains("ollama") {
            return "Ollama".into();
        }
    }
    if headers
        .keys()
        .any(|k| k.as_str().to_lowercase().starts_with("x-llama-cpp"))
    {
...
pub async fn probe(input: &ProbeInput) -> Result<Probed, ProbeError> {
    ...
    let resp = req.send().await.map_err(|e| {
        if e.is_timeout() {
            return ProbeError::Timeout(Duration::from_secs(10));
        }
        if e.is_connect() {
            return ProbeError::Unreachable(e.to_string());
        }
        if e.to_string().to_lowercase().contains("tls") {
            return ProbeError::Tls(e.to_string());
        }
    ...
    let json: serde_json::Value =
        serde_json::from_str(&body).map_err(|e| ProbeError::BadResponse(e.to_string()))?;
    let arr = json
        .get("data")
