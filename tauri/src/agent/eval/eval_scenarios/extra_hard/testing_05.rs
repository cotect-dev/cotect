...
"#).unwrap();
        ...
        with_blocked(with_scope(with_checks(pf(format!(
            "The banking system in {}, {}, {}, and {} may have bugs. \
             Your task is to write a comprehensive test file `test_banking.py` \
             that tests all functions and classes across all four modules \
             according to their docstrings.\n\n\
             Step 1: Read all source files and their docstrings carefully.\n\
             Step 2: Write `test_banking.py` with thorough tests. \
             Print \"ALL_TESTS_PASSED\" at the end if all assertions pass.\n\
             Step 3: Run `python3 test_banking.py` to see which tests catch bugs.",
            ...
            vec![account_file.clone(), transfer_file.clone(), interest_file.clone(),
                 statement_file.clone(), ap(dir, "test_banking.py")]),
