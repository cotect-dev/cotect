...
export type PanelGroup = 'git' | 'dev'
...
function cloneSlice(state: LayoutSlice): LayoutSlice {
  ...
  return {
    panels: {
      left: cloneZone(state.panels.left),
      right: cloneZone(state.panels.right),
      bottom: cloneZone(state.panels.bottom),
    },
    sizes: {
...
function findGroupLocation(
  panels: Record<PanelPosition, string[][]>,
  panelId: string,
): { position: PanelPosition; groupIndex: number } | null {
  for (const pos of POSITIONS) {
    for (let gi = 0; gi < panels[pos].length; gi++) {
...
function insertGroupIntoZone(
  ...
  if (slice.panels[position].length === 0) {
    ...
    const nIdx =
      neighborIndex ??
      (insertIndex < slice.panels[position].length
        ? insertIndex
        : slice.panels[position].length - 1)
...
export const useLayoutStore = createStoreWithHMR(import.meta.hot, 'layout', () =>
  create<LayoutState>((set) => ({
    ...
    movePanelToTab: (panelId, targetPanelId) =>
      set((state) => {
        ...
        if (srcLoc.position === tgtLoc.position && srcLoc.groupIndex === tgtLoc.groupIndex)
          return state
