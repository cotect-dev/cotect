...
export default function Layout({ mode = 'main' }: LayoutProps) {
  ...
  const leftVisible =
    isPanel || (crossWindowDrag?.position === 'left' ? 1 : 0) + effectiveCount('left') > 0
  const rightVisible =
    isPanel || (crossWindowDrag?.position === 'right' ? 1 : 0) + effectiveCount('right') > 0
  const bottomVisible =
    !isPanel && (crossWindowDrag?.position === 'bottom' ? 1 : 0) + effectiveCount('bottom') > 0
    ...
    <PanelHostProvider>
      <div className="w-screen h-screen flex flex-col">
        ...
        >
          <div
            ref={containerRef}
            className="relative flex-1 min-h-0 w-full flex flex-col pointer-events-none"
          >
              ...
              {isPanel ? (
                !anyDragging && (
                  <ResizeHandle
                    ...
                    onResizeEnd={(pixelLeft, pixelRight) => {
                ...
                <>
                  {leftVisible && !anyDragging && (
                    <ResizeHandle
                      mode="target"
                      orientation="vertical"
                      targetRef={leftZoneRef}
                      containerRef={topRowRef}
                      min={MIN_SIDE_ZONE}
                      max={0.4}
                      onResizeEnd={commitSize('left')}
                    />
                  )}
