    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-muted-foreground">
          {filtered.length} of {detected.models.length}
        </span>
      </div>
        ...
        <div className="min-w-[420px]">
          ...
          {filtered.slice(0, MAX_VISIBLE_MODELS).map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="grid grid-cols-[1fr_64px_64px_64px_72px] gap-0 px-2.5 py-1.5 border-t border-border text-[11px] font-mono items-center"
              >
                ...
                <div className="flex gap-1">
                  {roles.map((r) => (
                    <span
                      key={r}
                      className="text-[9px] bg-primary/20 text-primary px-1 py-0 rounded"
                    >
                      {r.slice(0, 4)}
                    </span>
                  ))}
