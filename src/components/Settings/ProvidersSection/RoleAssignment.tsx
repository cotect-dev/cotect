...
function readSlot(
  a: ActiveAssignment | null,
  key: RoleKey,
): { providerId: string | null; model: string | null } {
  if (!a) return { providerId: null, model: null }
  switch (key) {
    case 'default':
      return { providerId: a.default_provider_id, model: a.default_model }
    case 'implement':
      return { providerId: a.implement_provider_id, model: a.implement_model }
    case 'research':
      return { providerId: a.research_provider_id, model: a.research_model }
    case 'plan':
      return { providerId: a.plan_provider_id, model: a.plan_model }
  }
}
...
function writeSlot(
  ...
  switch (key) {
    case 'default':
      return { ...a, default_provider_id: providerId, default_model: model }
    case 'implement':
      return { ...a, implement_provider_id: providerId, implement_model: model }
    case 'research':
      return { ...a, research_provider_id: providerId, research_model: model }
    case 'plan':
      return { ...a, plan_provider_id: providerId, plan_model: model }
  }
}
...
const EMPTY: ActiveAssignment = {
  ...
  default_model: null,
  implement_provider_id: null,
  implement_model: null,
  research_provider_id: null,
  research_model: null,
...
export default function RoleAssignment({
  ...
  const onPick = async (key: RoleKey, modelId: string) => {
    const next =
      modelId === '__unset'
        ? writeSlot(a, key, null, null)
        : writeSlot(a, key, provider.id, modelId)
    ...
    <div className="flex flex-col gap-1.5 text-[11px]">
      ...
      <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 items-center">
        {ROLES.map((role) => {
          ...
          const showFallback =
            role !== 'default' && !isThisProvider && a.default_provider_id != null
          return (
