...
export default function ProviderCard({ provider, assignment }: { provider: Provider; assignment: ActiveAssignment | null }) {
  ...
  const dotColor =
    health.state === 'Healthy'   ? 'bg-green-500 shadow-green-500/50' :
    health.state === 'Degraded'  ? 'bg-yellow-500 shadow-yellow-500/50' :
    ...
    try {
      ...
      await upsert({
        ...
        health_json: JSON.stringify({
          ...
          last_ok_at_ms: Date.now(), p50_first_token_ms: health.p50_first_token_ms, last_error: null,
        }),
      })
    } catch (e) {
      await upsert({
        ...
        health_json: JSON.stringify({
          ...
          last_ok_at_ms: health.last_ok_at_ms, p50_first_token_ms: health.p50_first_token_ms,
          last_error: String(e),
        }),
      })
    } finally {
        ...
        {detected && (
          <>
            ...
            <div className="flex flex-wrap gap-1.5">
              <span className="px-2 py-0.5 rounded text-[10px] bg-muted">{detected.server_type}</span>
              {detected.capabilities.map((c) => (
                <span key={c} className="px-2 py-0.5 rounded text-[10px] bg-muted">{c}</span>
              ))}
            </div>
          </>
        )}
